...
void XtarkDriver::dynamic_reconfig_callback(xtark_driver::PID_reconfigConfig &config,uint32_t level)
{
   if(first_init_)
   {
	    ROS_INFO("Set PID P:[%d], I:[%d], D:[%d] ServoBias:[%d]",config.Kp,config.Ki,config.Kd,config.ServoBias);
    ...
    {
	    first_init_=true;
	    ROS_INFO("Set PID P:[%d], I:[%d], D:[%d] ServoBias:[%d]",kp_,ki_,kd_,servo_bias_);
    }   
}
/*主循环函数*/
void XtarkDriver::loop()
{
    ...
    ros::Subscriber imageSub = n.subscribe("/camera/image_raw", 1, &imageCB);
...
void XtarkDriver::send_speed_callback(const ros::TimerEvent&,const sensor_msgs::ImageConstPtr& msg)
{
